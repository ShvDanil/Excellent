...
* **Gateway.** Эта система отвечает за получение запроса с данными с сервера платформы и передачу запроса и данных микросервису для дальнейшей обработки, то есть распределением и маршрутизацией (более подробную информацию о работе API Gateway Pattern вы найдете ниже).
...
* **Test samples Database.** Это база данных, которая содержит информацию о пользователях, примерах их тестов с ответами.

...
* **Обработчик данных (Data Handler).** Эта подсистема, получая данные, осуществляет работу над ними и передает для анализа.
...
Один из [основных источников](https://microservices.io/patterns/monolithic.html) о монолитной архитектуре сообщает о том, что данный подход основан на принципе разделения ответственностей. ПО разделяется на слои, лежащие друг на друге. Каждый слой выполняет закрепленную за собой обязанность.

Перед нами возник вопрос, правильный ли выбор этой архитектуры? Если да, почему?

Проанализировав компоненты (слои), поняв то, как любая EdTech платформа будет взаимодействовать с нашим API, мы осознали, что крайне важной особенностью нашего продукта является его быстрая работа: пользователь, выполнив запрос, должен практически сразу же получить желаемый результат. А слабая связанность слоев повысит управляемость и скорость работы API. Также изолирование слоев поможет защитить одни слои от изменений, произведенных в других.
...
**Слой представления** содержит пользовательский интерфейс. Его реализацией будет заниматься платформа, использующая наш API.

**Слой бизнес-логики** содержит бизнес-логику программного обеспечения. Также он отделяет UI/UX от вычислений, связанных с бизнесом. Это позволяет без труда изменять логику слоя в зависимости от меняющихся бизнес-требований, не влияя при этом на другие слои.

**Слой передачи данных** отвечает за взаимодействие с базой данных API, обеспечивая хранение и обработку информации, которая не связана с бизнесом.

Теперь посмотрим на то, как выглядит [монолитная архитектура и ее слои](https://github.com/ShvDanil/Excellent/blob/main/readme_images/monolithic_architecture.png) схематично:
...
Для чего нашему API необходима эта архитектура и ее конкретные компоненты? Предлагаем проведем анализ описанных выше элементов.
...
С помощью **сервисного репозитория (Service Repository catalogue of services)** ESB направляет запрос в специальный сервис, который взаимодействует с другим сервисом (сервисами), чтобы подготовить данные для ответа.
...
Однако на этом не все. Данные должны где-то храниться, их должны откуда-то доставать и использовать. Перейдем к следующему виду референсной архитектуры.

### Database per Service architecture
Один из [основных источников](https://microservices.io/patterns/data/database-per-service.html) о **Database per Service** архитектуре описывает ее взаимодействие с внешними компонентами API. 

Доступ к базе данных API предоставляется с помощью последнего слоя монолитной архитектуры ("Слой передачи данных"): идет запрос в блок хранения данных (или базу данных API), где содержатся нужные таблицы с необходимыми данными для отработки запроса и предоставления ответа на него.

...
* **Контроллер (Controller).** Эта подсистема, которая получая запрос от обработчика запросов обрабатывает его и направляет в соответсвующую подсистему с данными для дальнейшей работы.
...
* **База данных клиента (Client Database).** Эта база данных содержит информацию о клиенте, от которого поступил запрос.

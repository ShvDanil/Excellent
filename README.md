...
Теперь стоит показать наши результаты на примере сервиса парсера теста для клиента по классу и предмету. Мы реализовали для него, как и для прочих микросервисов Swagger документацию, которая выглядит вот так:
